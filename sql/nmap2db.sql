...
CREATE TABLE scan_definition(
  ...
  parsemethod TEXT NOT NULL DEFAULT 'nmap_default',
  args TEXT NOT NULL,
  remarks TEXT
);
...
CREATE TYPE scan_scope AS ENUM ('full', 'live', 'liveport');
...
CREATE TABLE scan_job(
  ...
  network_addr CIDR NOT NULL,
  scan_id TEXT NOT NULL,
  scan_scope scan_scope NOT NULL DEFAULT 'full',
);
...
-- Table: ssl_report
--
-- @Description: Information about the scans executed by testssl. All
--		 the information in this table is extracted
--		 automatically when saving the XML output of a NMAP
--		 scan executed with the -oX paramater.
--
-- Attributes:
--
-- @report_id: MD5 value of the XML output of this report
...
-- @finished: Timestamp when the scan was finished (/nmaprun/runstats/finished/@time)
-- @elapsed_time: Scan duration (/nmaprun/runstats/finished/@elapsed)
-- @common_name: Scan type (nmaprun/scaninfo/@type)
-- @issuer: Scan protocol (/nmaprun/scaninfo/@protocol)
-- @support_ssl: Number of services scanned (/nmaprun/scaninfo/@numservices)
-- @support_tls: Arguments used by NMAP to run the scan (/nmaprun/@args)
-- @expiry: NMAP version used to run the scan (/nmaprun/@version)
-- @vulnerable: Version of the XML output (/nmaprun/@xmloutputversion)
-- @jsonreport: JSON output of the scan
--
-- ------------------------------------------------------------

\echo '\n# [Creating table:ssl_report]\n'

CREATE TABLE ssl_report(
  report_id TEXT NOT NULL,
  registered TIMESTAMP WITH TIME ZONE NOT NULL DEFAULT now(),
  scan_jobid BIGINT,
  started TIMESTAMP WITH TIME ZONE,
  ...
  issuer TEXT,
  issued TEXT,
  support_ssl BOOLEAN DEFAULT FALSE,
  support_tls BOOLEAN DEFAULT FALSE,
  expiry TEXT,
  vulnerable BOOLEAN DEFAULT FALSE,
  jsonreport JSON
);

ALTER TABLE ssl_report ADD PRIMARY KEY (report_id);
ALTER TABLE ssl_report OWNER TO nmap2db_role_rw;

CREATE INDEX ssl_report_registered_idx ON ssl_report(registered);
CREATE INDEX ssl_report_started_idx ON ssl_report(started);
CREATE INDEX ssl_report_finished_idx ON ssl_report(finished);
CREATE INDEX ssl_report_elapsed_time_idx ON ssl_report(elapsed_time);
...
-- ------------------------------------------------------------
-- Function: get_scan_job_command()
--
-- Parameters:
-- @scan_job_id (BIGINT): Scan job ID
--
-- Return: scan command for a scan job
-- ------------------------------------------------------------

\echo '\n# [Creating function get_scan_job_command]\n'

CREATE OR REPLACE FUNCTION get_scan_job_command(BIGINT) RETURNS TEXT
LANGUAGE sql
SECURITY INVOKER
SET search_path = public, pg_temp
AS $$
...
ALTER FUNCTION get_scan_job_command(BIGINT) OWNER TO nmap2db_role_rw;

-- ------------------------------------------------------------
-- Function: get_scan_job_parsemethod()
--
-- Parameters:
-- @scan_job_id (BIGINT): Scan job ID
--
-- Return: prefered parse method for a scan job
-- ------------------------------------------------------------

\echo '\n# [Creating function get_scan_job_parsemethod]\n'

CREATE OR REPLACE FUNCTION get_scan_job_parsemethod(BIGINT) RETURNS TEXT
LANGUAGE sql
SECURITY INVOKER
SET search_path = public, pg_temp
AS $$

  SELECT b.parsemethod
  FROM scan_job a
  INNER JOIN scan_definition b
  ON a.scan_id = b.scan_id
  WHERE a.id = $1;
...
ALTER FUNCTION get_scan_job_parsemethod(BIGINT) OWNER TO nmap2db_role_rw;
...
\echo '\n# [Creating function save_scan_report_xml]\n'

CREATE OR REPLACE FUNCTION save_scan_report_xml(scan_jobid BIGINT, xmlreport XML) RETURNS VOID
LANGUAGE plpgsql
SECURITY INVOKER
SET search_path = public, pg_temp
AS $$
...
ALTER FUNCTION save_scan_report_xml(BIGINT,XML) OWNER TO nmap2db_role_rw;

-- ------------------------------------------------------------
-- Function: save_scan_report_json()
--
-- Parameters:
-- @jsonreport (JSON): JSON report from a NMAP scan
--
-- Return: Report ID for the scan report saved in the database
-- ------------------------------------------------------------

\echo '\n# [Creating function save_scan_report_json]\n'

CREATE OR REPLACE FUNCTION save_scan_report_json(scan_jobid BIGINT, jsonreport JSON) RETURNS VOID
LANGUAGE plpgsql
SECURITY INVOKER
SET search_path = public, pg_temp
AS $$
BEGIN
 ...
 --
  ...
  EXECUTE 'INSERT INTO scan_report (scan_jobid, jsonreport) VALUES ($1,$2)'
  USING scan_jobid,
	lower(jsonreport::text)::JSON;

  RETURN;
END;
$$;

ALTER FUNCTION save_scan_report_json(BIGINT, JSON) OWNER TO nmap2db_role_rw;

-- ------------------------------------------------------------
-- Function: save_ssl_report_json()
--
-- Parameters:
-- @jsonreport (JSON): JSON report from a testssl scan
--
-- Return: Report ID for the scan report saved in the database
-- ------------------------------------------------------------

\echo '\n# [Creating function save_ssl_report_json]\n'

CREATE OR REPLACE FUNCTION save_ssl_report_json(scan_jobid BIGINT, jsonreport JSON) RETURNS VOID
LANGUAGE plpgsql
SECURITY INVOKER
SET search_path = public, pg_temp
AS $$
BEGIN
 ...
 --
  ...
  EXECUTE 'INSERT INTO ssl_report (scan_jobid, jsonreport) VALUES ($1,$2)'
  USING scan_jobid,
	lower(jsonreport::text)::JSON;
...
ALTER FUNCTION save_ssl_report_json(BIGINT, JSON) OWNER TO nmap2db_role_rw;
